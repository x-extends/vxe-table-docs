    ...
    return {
      ...
      gridOptions: {
        ...
        columns: [
          ...
          { field: 'defVal', title: 'app.api.title.defVal', type: 'html', minWidth: 160, titlePrefix: { content: '部分参数可支持全局设置，具体请查阅相关说明' } },
          { field: 'version', title: 'app.api.title.version', type: 'html', width: 120, titlePrefix: { content: '该文档与最新版本保持同步，如果遇到参数无效时，\n请检查当前使用的版本号是否支持该参数' }, slots: { default: 'default_version' } }
        ],
        data: []
      }
    }
    ...
    handleSearch () {
      ...
      if (filterName) {
        ...
        if (/pro/i.test(filterName)) {
          const rest = XEUtils.searchTree(this.tableData, item => item.version === 'extend-cell-area', options)
