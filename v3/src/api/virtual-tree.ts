    ...
    list: [
      {
        ...
        list: XEUtils.clone((toolbarAPI as any[]).find(item => item.name === 'Props').list, true).concat([{
          ...
          enum: '',
          defVal: '',
          list: XEUtils.clone((toolbarAPI as any[]).find(item => item.name === 'Slots').list, true)
        }])
      },
      {
        name: 'pager-config',
        ...
        enum: '',
        defVal: '',
        list: XEUtils.mapTree((pagerAPI as any[]).find(item => item.name === 'Props').list.filter((item: any) => !['size', 'loading'].includes(item.name)), item => Object.assign({}, item, { name: XEUtils.camelCase(item.name) }))
      },
      {
        name: 'proxy-config',
...
const gridAPI = XEUtils.clone(tableAPI, true).map(item => {
  ...
  rest.list = (item.list as any[]).concat(rest.list)
  return rest
})
...
gridAPI.find(item => item.name === 'Props').list.splice(1, 0, {
  ...
  enum: '',
  defVal: '',
  list: XEUtils.mapTree((columnAPI as any[]).find(item => item.name === 'Props').list, item => Object.assign({}, item, { name: XEUtils.camelCase(item.name) }))
})
