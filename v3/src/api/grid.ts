    ...
    list: [
      {
        ...
        list: XEUtils.clone((formAPI as any[]).find(item => item.name === 'Props'), true).list.map((item: any) => Object.assign({}, item, { name: XEUtils.camelCase(item.name) })).concat([
          ...
          },
          {
            ...
            enum: '',
            defVal: '',
            list: XEUtils.clone((formItemAPI as any[]).find(item => item.name === 'Props'), true).list.map((item: any) => Object.assign({}, item, { name: XEUtils.camelCase(item.name) })).concat([
              formItemSlots
            ])
          }
        ])
...
XEUtils.eachTree((gridAPI as any[]), (item, index, obj, paths, parent) => {
...
gridAPI.find((item: any) => item.name === 'Props').list.splice(1, 0, {
  ...
  enum: '',
  defVal: '',
  list: XEUtils.mapTree((columnAPI as any[]).find(item => item.name === 'Props').list, item => Object.assign({}, item, { name: XEUtils.camelCase(item.name) })).concat([
    {
      name: 'children',
      desc: '表头分组',
      version: '',
