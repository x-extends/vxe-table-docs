    ...
    <CodeLight path="table/edit/status/Demo1">
      <template #tip>
        <vxe-tip status="primary" title="编辑状态">
          设置 <ApiLink name="table" prop="keep-source"/> 开启保持原始值状态和 <ApiLink name="table" prop="edit-config"/>.<ApiLink name="table" prop="showStatus"/> 和 <ApiLink name="table" prop="edit-render"/> | <ApiLink name="table" prop="cell-render"/> 开启编辑状态显示功能<br>
          对于某些需要局部保存的场景，可以在数据保存完成后调用 <ApiLink name="table" prop="reloadRow"/> 方法加载行数据并恢复到初始状态<br>
        </vxe-tip>
        <vxe-tip status="warning" title="小提示">
          如果使用自定义模板，列设置 <ApiLink name="column" prop="edit-render"/> | <ApiLink name="column" prop="cell-render"/> 并开启 <ApiLink name="table" prop="edit-config"/>.<ApiLink name="table" prop="showStatus"/> 后再配合 <ApiLink name="table" prop="updateStatus"/> 方法使用，在对应单元格的值发生改变时调用更新状态。
