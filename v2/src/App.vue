<template>
  <div id="app" @click="clickEvent">
    <div class="page-top">
      <span>Vxe table 2.x 版本在 2021-12-01 起终止支持且不再维护。建议使用 <a class="link" href="https://vxetable.cn/v3.8/">Vxe UI 3.9+</a> 版本，避免出现严重问题及漏洞！</span>
    </div>
...
export default {
  ...
  created () {
    ...
    if (process.env.NODE_ENV !== 'development') {
      if (!sessionStorage.getItem('READ_OLD_VERSION_UPGRADE_2')) {
        VXETable.modal.alert({
          ...
          if (type === 'confirm') {
            sessionStorage.setItem('READ_OLD_VERSION_UPGRADE_2', '1')
          }
        })
      }
    }
