<template>
  <div class="app-container" @click="clickEvent">
    <header class="page-header">
      ...
      <div class="right">
        <div class="content">
          ...
          <vxe-select class="version-switch" size="mini" v-model="appData.compVersion" :options="sysVersionOptions" @change="vChangeEvent"></vxe-select>
            ...
            <template v-if="appData.apiList.length">
              <ul class="nav-menu">
                <li v-for="(item, index) in appData.apiList" :key="index" :class="{expand: item.expand}">
                  <a v-if="item.linkUrl" :href="item.linkUrl" target="_blank" class="link" style="padding-left: 20px;">{{ item.label }}</a>
    ...
    <vxe-modal v-model="showUpgrade" title="重要提醒，由于 v4.7 版本的重构" :width="540" resize show-zoom show-footer show-cancel-button show-confirm-button confirm-button-text="忽略，不再提醒" @confirm="confirmUpgrade">
...
const showUpgrade = ref(!localStorage.getItem('SHOW_4D6_UPGRADE'))
...
const selectSysVersion = computed(() => {
  return systemVersionList.value.find(item => item.version === appData.compVersion)
})
...
const getVersion = () => {
  fetch(`${serveApiUrl.value}/api/npm/versions/vxe-table`, { method: 'GET' })
    ...
    .then((data) => {
      ...
      const stableVersionList = data[`v${appData.docsVersion}StableList`].map(value => ({ value, label: value }))
      const betaVersionList = data[`v${appData.docsVersion}BetaList`].map(value => ({ value, label: value }))
      appData.stableVersionList = stableVersionList
      appData.betaVersionList = betaVersionList
      if (stableVersionList.length) {
        appData.selectStableVersion = tags[`v${appData.docsVersion}-legacy`] || stableVersionList[0].value
      }
...
const confirmUpgrade = () => {
  localStorage.setItem('SHOW_4D6_UPGRADE', '1')
}
