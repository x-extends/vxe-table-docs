...
const check4ChangeEvent = (row: any, checked: boolean) => {
  let childList = gridOptions.data.filter(item => item.id3 === row.id3 && item.name4 === row.name4)
  childList.forEach(item => {
    item.check4 = checked
  })
  childList = gridOptions.data.filter(item => item.id3 === row.id3)
  const isChecked3 = childList.every(item => item.check4)
  childList.forEach(item => {
    item.check3 = isChecked3
  })
  childList = gridOptions.data.filter(item => item.id2 === row.id2)
  const isChecked2 = childList.every(item => item.check3)
  childList.forEach(item => {
    item.check2 = isChecked2
  })
  childList = gridOptions.data.filter(item => item.id1 === row.id1)
...
const check3ChangeEvent = (row: any, checked: boolean) => {
  let childList = gridOptions.data.filter(item => item.id2 === row.id2 && item.name3 === row.name3)
  childList.forEach(item => {
    item.check3 = checked
  })
  childList = gridOptions.data.filter(item => item.id3 === row.id3)
...
const check2ChangeEvent = (row: any, checked: boolean) => {
  let childList = gridOptions.data.filter(item => item.id1 === row.id1 && item.name2 === row.name2)
  childList.forEach(item => {
    item.check2 = checked
  })
  childList = gridOptions.data.filter(item => item.id2 === row.id2)
...
const check1ChangeEvent = (row: any, checked: boolean) => {
  let childList = gridOptions.data.filter(item => item.name1 === row.name1)
  childList.forEach(item => {
    item.check1 = checked
  })
  childList = gridOptions.data.filter(item => item.id1 === row.id1)
