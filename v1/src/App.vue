<template>
  <div id="app" @click="clickEvent">
    <div class="page-top">
      <span>Vxe table 1.x 版本在 2020-04-01 起终止支持且不再维护。建议使用 <a class="link" href="https://vxetable.cn/v3.8/">Vxe UI 3.9+</a> 版本，避免出现严重问题及漏洞！</span>
    </div>
...
export default {
  ...
  created () {
    ...
    if (process.env.NODE_ENV !== 'development') {
      if (!sessionStorage.getItem('READ_OLD_VERSION_UPGRADE_1')) {
        VXETable.modal.alert({
          title: '重要公告：v1.x 停止维护',
          message: 'vxe-table v1.x 版本于 2020 年 4 月 1 日起停止维护。建议使用最新版本。',
          status: 'warning',
          confirmButtonText: '我已知晓'
        }).then((type) => {
          if (type === 'confirm') {
            sessionStorage.setItem('READ_OLD_VERSION_UPGRADE_1', '1')
          }
        })
      }
    }
